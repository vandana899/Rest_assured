...
public void tc2() {
	...
	given().get(url).then().statusCode(200).body("data.id[0]", equalTo(7));// getting single user
...
public void tc3() {
	...
	given().body(js.toJSONString()).post(url).then().statusCode(201).log().all();
}

@Test(enabled = false)
public void tc4() {
	given().delete("https://reqres.in/api/users/2").then().statusCode(204).log().all();//deleting the user
}
